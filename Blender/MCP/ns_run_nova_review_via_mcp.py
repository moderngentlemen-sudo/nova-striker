"""Run Nova's automated review through the standard MCP stdio protocol."""
...
async def run(args):
    ...
    live_script = repo_root / "Blender" / "MCP" / "ns_nova_mcp_live_review.py"
    ...
    if not live_script.is_file():
        raise RuntimeError(f"Missing Nova MCP live review script: {live_script}")
...
"""
            ...
            if review_result.isError:
                raise RuntimeError(tool_text(review_result))

            review_output = tool_text(review_result).strip()
            proof = {
                "schema_version": 1,
                ...
                "live_review_output": review_output,
            }
            ...
            print("MCP-DRIVEN NOVA REVIEW PASSED")
